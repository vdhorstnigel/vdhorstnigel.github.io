...
#### Limitations of the analog sensors 
Unfortunately, the sensors are qualitative not quantitative.This means that its unable to measure the VOC content accurately and can only read if its increasing or decreasing. However, this will be addressed by using machine learning, we only need to look at the trend of the data, we do not need the exact number to measure the VOC and access if the food is spoiling.
...
## Machine Learning
For Machine learning, I want to do real-time anomaly detection based on the VOC data similar to how netdata does its anamoly detection.
Reference here: https://github.com/netdata/netdata/blob/master/src/ml/notebooks/netdata_anomaly_detection_deepdive.ipynb
Netdata uses the dlib c++ library and the find_cluster_using_kmeans function along with others so this is great for our use case.
Netdata K-means code: https://github.com/netdata/netdata/blob/master/src/ml/ml_kmeans.cc
Unfortunately, the code is in c++ and difficult to link to C, however we can still use the concepts listed to build our own anamoly detection.
I am using this library from pramsey: https://github.com/pramsey/kmeans, which is a simple C routine for k-means calculation which also allows multithreading for better perfomance.
I can also look into using edge impulse for edge computing.
Another K-means article: https://amid.fish/anomaly-detection-with-k-means-clustering

### Unsupervised machine learning algorithms

I had the idea of anomaly detection via unsupervised machine learning when using netdata as a network monitoring system. I thought that this would work great for recognizing food spoilage in the fridge. This is because all fridges are different and every fridge baseline VOC level might be different. Some people might have a higher baseline VOC level due to the various foods present. This allows the algorithmn to access the VOCS in the fridge then find anamolies when VOCs spike.

#### Dimensions
We have 4 sensors which means we would have 4 dimensions to process. Typically, most guides would recommend to do dimensionality reduction, but is it really needed? Typically, the most common method would be PCA which uses covariance to find eigen vectors keeping the data with the best features. This allows our data to be reflected as a 2 dimensional or 3 dimensional matrix, allowing better visulization and multicollinearity. However, the draw back is that we might overfit our model or might inject false correlations. Lets say that initially that fridge has only fruits and vegetables, PCA will place more weights on ethylene and ethanol as those values would change the most. If meat was to be introduct, the model may not pick up that the spoiling meat. We can do PCA on every iteration before doing our machine learning but that is very computationally expensive and wouldnt make sense. So the solution is to just process each dimension one at a time, while we might not be able to pick up on correlation, we can accurately detect when anomalies are present for each dimension. This is how netdata does it as well.

##### K-Means
K-means is the basic clustering algorithm used for unsupervised machine learning and it is also very simple to implement. We initially scale the data and select a random point on that scale. Typically K-means would be done on 2 dimensional or 3 dimensional datasets, finding the euclidean distance from the our dataset point to the intial random point. But since our dataset is only one dimensional, we just need to find the linear distance. Then we would calculate all the distance and find the mean. We then keep recalculating a new point reducing the mean as much as possible to find the best possible centoid. We can then detect anomalies by calculating how far the point is from the centoid based on the standard deviation or z-score.

##### DBSCAN
Density Based Spatial Clustering of Applications with Noise is also another popular unsupervised machine learning clustering algorithm. The difference is that DBSCAN has creates points which must have a minimum number of points within a given radius. This algorithm is extremly efficient in detecing outliers and handling noise. But since we are using one dimensional data that isnt very noisy, DBScan is abit too overkill for our application. We also need to account that we have very little memory and need our calculations to be as streamlined as possible if we want to include a longer window to train.

#### Autoencoder
Autoencoder is also a unsupervised machin learning algorithm but uses artifical neural network. It works by compressing and coding that data, then learning how to reconstruct the data from the reduced encoded representation to a representation that is as close to the original input as possible. This is great for identifying patterns and finding outliers that do not conform to the pattern, like finding a heartbeat anomaly in a EKG report. But since our data does not really conform to a pattern, this is not very suitable for this project.

### FreeRTOS
I should be looking into FreeRTOS for scheduling and running task in parallel instead of using a linear loop with vtaskdelay to allow accurate reading every second.

### Edge computing
Why is edge computing important? With machine learning and AI become some prevalent, many people know that training the model is computationally expensive and also uses alot of energy. However, many people also dont know that inferring from the model also uses alot of energy when transmitting or sending data on the internet. That is why edge computing is getting more popular now to reduce load on the network and have models run locally at the edge device.
